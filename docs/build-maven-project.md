...
jobs:
  build-maven-project:
    uses: wtaxco/wtax-github-actions-workflows/.github/workflows/build-maven-project.yml@testing
    with:
...
| `build-artifact-name`          | Input          | `string` | The name of the artifact containing the build output from all the target directories in the project that this job will create and upload using the `upload-artifact` action. It can be downloaded in subsequent jobs in the same workflow using the `download-artifact` action, specifying the artifact name specified by this input.         | artifact                                       |
| `deploy-scripts-artifact-name` | Input          | `string` | The name of the artifact containing the contents of the project's deploy directory that this job will create and upload using the `upload-artifact` action. It can be downloaded in subsequent jobs in the same workflow using the `download-artifact` action, specifying the artifact name specified by this input.                          | deploy-scripts                                 |
